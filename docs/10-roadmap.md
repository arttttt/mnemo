...
daemon, no Docker), on SQLite + `sqlite-vec` + FTS5; an agent works through `remember` / `search` / `browse` / `delete` / `recall` (`recall` is the one opt‑in LLM read tool; the write path stays LLM‑free).
