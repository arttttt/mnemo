...
### `recall(query, project, limit?) -> {project, summary, sources}` — opt‑in LLM synthesis
...
so the answer stays light on the caller's context. `limit` is the number of most‑relevant memories to ground on.
Unlike `search` (ranked hits) it returns a written answer. The write path stays LLM‑free; `recall` is the single
opt‑in LLM read tool, and the generator is **transient** (loaded on demand, unloaded after). Disable synthesis
entirely with `MNEMO_GENERATOR=off`.
