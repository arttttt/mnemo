"""In-memory project registry double for unit tests (no I/O).

A fast, dependency-free test double of ProjectRepository (the SQLite registry is
the real one). The reserved `__global__` sentinel is always present — exempt from
the gate, hidden from listings.
"""
...
class InMemoryProjectRepositoryImpl:
    def __init__(self) -> None:
        self._projects: dict[str, Project] = {
            GLOBAL_PROJECT: Project(GLOBAL_PROJECT, None, now())
        }
